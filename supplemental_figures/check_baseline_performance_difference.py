...
def evaluate_positive_difference(result_sleep_stage, result_eeg, labels_model_baseline, threshold=0.25):
    
    ## first filter to only the filenames in result_eeg
    labels_model_baseline = labels_model_baseline[labels_model_baseline['filename'].isin(result_eeg['filename'])]
    result_eeg.rename(columns={'prob': 'eeg_prob'}, inplace=True)
    result_eeg = result_eeg[['filename', 'eeg_prob', 'fold']]
    result_sleep_stage.rename(columns={'prob': 'sleep_stage_prob'}, inplace=True)
    result_sleep_stage = result_sleep_stage[['filename', 'sleep_stage_prob', 'fold']]
    result_sleep_stage = result_sleep_stage.groupby('filename', as_index=False).agg('mean').reset_index()
    result_eeg = result_eeg.groupby('filename', as_index=False).agg('mean').reset_index()
    
    result_sleep_stage = result_sleep_stage.merge(result_eeg, on='filename', how='inner')
    # result_sleep_stage['filename'] = result_sleep_stage['filename'].apply(lambda x: x.replace('wsc-visit5', 'wsc').replace('wsc-visit4', 'wsc').replace('wsc-visit3', 'wsc').replace('wsc-visit2', 'wsc').replace('wsc-visit1', 'wsc'))
    # labels_model_baseline['filename'] = labels_model_baseline['filename'].apply(lambda x: x.replace('wsc-visit5', 'wsc').replace('wsc-visit4', 'wsc').replace('wsc-visit3', 'wsc').replace('wsc-visit2', 'wsc').replace('wsc-visit1', 'wsc'))
    
    labels_model_baseline = labels_model_baseline.merge(result_sleep_stage, on='filename', how='inner')
    
    labels_model_baseline = labels_model_baseline[labels_model_baseline['label'] == 1]
    sleep_stage_fn = labels_model_baseline[labels_model_baseline['sleep_stage_prob'] < threshold].copy()
    sleep_stage_tp = labels_model_baseline[labels_model_baseline['sleep_stage_prob'] > threshold].copy()
    sleep_stage_fn_corrected = sleep_stage_fn[sleep_stage_fn['prob'] > threshold].copy()
    
    eeg_tp = labels_model_baseline[labels_model_baseline['eeg_prob'] > threshold].copy()
    eeg_fn = labels_model_baseline[labels_model_baseline['eeg_prob'] < threshold].copy()
    eeg_fn_corrected = eeg_fn[eeg_fn['prob'] > threshold].copy()
    
    eeg_corrections_probabilities = eeg_fn_corrected['taxonomy'].value_counts(normalize=True)
    sleep_stage_corrections_probabilities = sleep_stage_fn_corrected['taxonomy'].value_counts(normalize=True)
    sleep_stage_tp_probabilities = sleep_stage_tp['taxonomy'].value_counts(normalize=True)
    eeg_tp_probabilities = eeg_tp['taxonomy'].value_counts(normalize=True)
    print(f"EEG corrections probabilities: {eeg_corrections_probabilities}")
    print(f"Sleep stage corrections probabilities: {sleep_stage_corrections_probabilities}")
    ## now compare to TP of both models 
    a1 = pd.DataFrame(sleep_stage_corrections_probabilities).reset_index()
    b1 = pd.DataFrame(sleep_stage_tp_probabilities).reset_index()
    a2 = pd.DataFrame(eeg_corrections_probabilities).reset_index()
    b2 = pd.DataFrame(eeg_tp_probabilities).reset_index()
    a1.rename(columns={'proportion': 'model_extra_proportion'}, inplace=True)
    b1.rename(columns={'proportion': 'sleep_stage_tp_proportion'}, inplace=True)
    a2.rename(columns={'proportion': 'model_extra_proportion'}, inplace=True)
    b2.rename(columns={'proportion': 'eeg_tp_proportion'}, inplace=True)
    ab1 = a1.merge(b1, on='taxonomy', how='inner',)
    ab2 = a2.merge(b2, on='taxonomy', how='inner',)
    ab1 = pretty_print(ab1)
    ab2 = pretty_print(ab2)
    dfi.export(ab1, 'sleep_stage_corrections_probabilities.png')
    dfi.export(ab2, 'eeg_corrections_probabilities.png')
    
def pretty_print(df):
    rename_taxonomy_dict = {
        'Nxxx': 'NTCA (unspecified)',
        'Txxx': 'TCA (unspecified)',
        'NSFx': 'Fluoxetine',
        'NSSx': 'Sertraline',
        'NSPx': 'Paroxetine',
        'NNVx': 'Venlafaxine',
        ...
        'NSEC': 'Citalopram',
        'NSEE': 'Escitalopram',
        'NSEx': 'Citalopram derivatives'
    }
    df = df[df['taxonomy'].isin(rename_taxonomy_dict.keys())]
    df = df[df['model_extra_proportion'] > 0.01]
    df['taxonomy'] = df['taxonomy'].apply(lambda x: rename_taxonomy_dict[x])
    ## round the values to 2 decimal places
    df['model_extra_proportion'] = df['model_extra_proportion'].round(2)
    if 'sleep_stage_tp_proportion' in df.columns:
        df['sleep_stage_tp_proportion'] = df['sleep_stage_tp_proportion'].round(2)
    if 'eeg_tp_proportion' in df.columns:
        df['eeg_tp_proportion'] = df['eeg_tp_proportion'].round(2)
    df.rename(columns={'taxonomy': 'Medication Type', 'model_extra_proportion': 'Transformer Added Proportion', 'sleep_stage_tp_proportion': 'Sleep Stage Correctly Predicted Proportion', 'eeg_tp_proportion': 'Sleep Stage + EEG Correctly Predicted Proportion'}, inplace=True)
    return df 
...
def main():
    ...
    evaluate_positive_difference(result_sleep_stage, result_eeg, labels_model_baseline)
