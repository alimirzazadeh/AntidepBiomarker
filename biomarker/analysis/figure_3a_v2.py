...
def load_taxonomy_data(labels):
    ...
    ## fix the pid for shhs 
    for dataset in ['wsc', 'mros', 'shhs']:
        mask = labels_model_baseline['dataset'] == dataset
        labels_model_baseline.loc[mask, 'pid'] = labels_model_baseline.loc[mask, 'pid'].apply(lambda x: x[1:] if isinstance(x, str) and x else x)
...
def run_transformer_baseline(train_set, train_y, test_set, test_y, cols=None):
    """
...
def run_cross_validation(df, df_eeg, model1_cols, model2_cols):
    ...
    for fold in range(4):
        ...
        run_transformer_baseline(train_features, train_y, test_features, test_y, cols=model1_cols)
        bp()
...
def main():
    ...
    result_sleep_stage = pd.read_csv(os.path.join(CSV_DIR,'baselinestage_inference_v6emb_5880_all.csv'))
    result_eeg = pd.read_csv(os.path.join(CSV_DIR,'baselineeegstage_inference_v6emb_5880_all.csv'))
    labels = pd.read_csv(os.path.join(CSV_DIR,'inference_v6emb_3920_all.csv'))
    result_sleep_stage = load_taxonomy_data(result_sleep_stage)
    result_eeg = load_taxonomy_data(result_eeg)
    labels_model_baseline = load_taxonomy_data(labels)
